...
function ResumeViewerModule() {
  ...
  const [statusPhase, setStatusPhase] = useState<"Loading" | "Rendering" | "Ready">("Loading");
  ...
  useEffect(() => {
    ...
    if (!pdfLoaded) {
      setStatusPhase("Rendering");
      return;
    }

    const timer = window.setTimeout(() => setStatusPhase("Ready"), 120);
    return () => window.clearTimeout(timer);
  }, [viewerReady, pdfLoaded]);
